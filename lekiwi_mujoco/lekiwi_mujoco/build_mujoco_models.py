...
def compose_scene(robot, scene='flat', instance=None):
    ...
    visual settings and lighting. No fleet, obstacle or terrain generator is defined.
    """
    no_scene = scene is False or scene is None or scene == 'none'
    if no_scene and instance is None:
        return robot
    instance = instance or RobotInstance()
    if no_scene:
        world = mujoco.MjSpec()
    else:
    ...
    world.attach(robot, prefix=instance.prefix, frame=world.worldbody.add_frame(
        name=instance.name + '_spawn', pos=instance.position, quat=instance.quaternion))
    return world


def build_spec(variant, pt_package=None, scene='flat', *, control_dir=None, description_dir=None, instance=None):
    return compose_scene(build_robot_spec(variant, pt_package, control_dir=control_dir, description_dir=description_dir), scene, instance)


def build(variant, output, absolute=False, pt_package=None, scene=True, *, control_dir=None, description_dir=None, instance=None):
    output = Path(output).resolve()
    spec = build_spec(variant, pt_package=pt_package, scene=scene, control_dir=control_dir, description_dir=description_dir, instance=instance)
    ...
    spec.compile()
    content = '<!-- Generated by scripts/build_mujoco_models.py using MjSpec; edit sources, not this file. -->\n'
...
def main():
    ...
    if args.variant:
        build(args.variant, args.output, args.absolute, args.pt_package, scene=args.scene, control_dir=args.control_package, description_dir=args.description_package, instance=instance)
    else:
        for variant in ('base', 'pt100', 'pt101'):
            filename = 'lekiwi_base.xml' if variant == 'base' else f'lekiwi_{variant}_oakd_s2.xml'
            print(build(variant, SIM_PACKAGE / 'mjcf' / filename, args.absolute, args.pt_package, scene=args.scene, control_dir=args.control_package, description_dir=args.description_package, instance=instance))
